...
\textbf{Correct}: This evaluation would suggest that there are many faults in this protoype that would need to be ironed out. Some of these issues would stem from underlying X problems (such as people not beign able to see files, which is caused by people misunderstanding hwo files are marked as private or final). There also seems to be some issue with respect to making sure that test cass can run succesfully - This would need to be fixed as part of the pipeline system  \todo{mention that this wasn't fully implemented before testing, and as part of conducting the signature check on solutions, youc ould do some sort of check ont est cases to make sure they were parseable... perhaps?}. Other than this, the behaviour of the site seemed to be consistemt with what was expected. \todo{Do i ever want to mention the lack of unit testing? I could mention it and discuss how as this si a prototype the overhead associate dwith producing tests mgiht be very high. Besides, I think the solution I ended up with is kind of ugly, and re-writing it migth allow for some unit -testing kind of situations now I know ore about what the whole thing is supposed to be doing}\par
\textbf{Usable}: There are many usability isses in the website. Partly stemming from the lack of nice appearance and layout, which might act psycologically to make people think its more complicated \todo{citaiton needed}. The UX of the site is also difficult. There are issues with onboarding, discoverability and general understanding of what each part of the site does (with the exception of the feedback view, which was found to be self-explanatory, once participants were able to find it). \todo{do we mention fixes for problem here or in the next chapter?}. In terms of the responsivenes of the website, it seemed to be performing well. The website seems to be lackign some common abilities one would expect from a website with social interaction, such as being able to edit comments in feedback, which is unexpected behaviour.\par
\textbf{Speed}: I am beginning to suspect that including this a an evaluation objetive was an optimistic choice. Due to the issues that arose during the conducting of the experiment (with some participants opting to leave the study early), and email responses only being produced at the time of the website going into feedback mode (possibly a mis-communication of the time slices in the initial email I sent out \todo{appendix this?}); I do not feel I have enough evidence to comment on any improvements or deteriorations in turnaround time for getting feedback through either method.\par
\textbf{Enhance}: Based on the feedback given regarding the need for 2-way discussions during feedback, I suspect that the email based feedback system outperforms the website. And due to the issues that ocurred with many tests not running fully, I feel that the feedback given on the site would obviously not be as detailed as one would expect.
Secure
...
% END
