...
To try and get as fair a response as possible, each participant in each group would complete two tasks, but the pairing of which task would be completed using the website or by email would be swapped between each group. Examples of the tasks that would be completed are given as appendices \todo{num here}.\par
Participants filled out a consent form, as can be seen in appendix \todo{number?}, then took part in the peer-testing exercises. Upon completing this, they were given a survey to complete and an invitation to a discussion session. Finally, participants were rewarded with a gift voucher.

\section{Participation}
During the study I had planned for a cohort of 16 people participating, however throughout the process a lower number, actually took part, with 2 participants withdrawing, and not all participants completing the peer-testing exercise. Overall, \todo{10?} people filled out the final survey.\par
As part of the final survey participants were requested to fill out, the following demographic data was found:
\begin{itemize}
\item The majority of participants were Computer Systems students, with \todo{6} participants. Then, \todo{2} Software Engineering students and \todo{1} Computer Science student.\footnote{Before 4th year, both the Software Engineers and Computer Scientists follow the same course.}
\item Most of those who took part were Year 2 students (\todo{6} participants), and only \todo{2} Year 3. 
\item There was a decent mix of students from both campuses, with \todo{5} from Edinburgh, and \todo{4} from Dubai.
\item There were more male participants, with \todo{6} identifying as male, and \todo{3} identifying as female.
\end{itemize}
I feel that the spread of students that ended up taking part does provide a useful range over the demographics within the university as a whole.\todo{is there any obvious difference in responses across different demographic groups?}\par
...
My observations - the website architecture is not very good. I definitely needs re-thinking in terms of how things work. For example, forcing people to submit stuff before a deadline doesn't seem optimal, but maybe thats what teachers want. what if students wwho finish early could peer test themselves while others finish later, withou having long dealys- that seems a good change that would be needed.\todo{wtf am I talking about?}\par
...
The teachers view was created with quite minimal functionality, with most focus given to the student view. This did cause some issues though, for example with regards to editing descriptor files for a givwn coursework - this is simply not supported and would of course be an addition that would be needed for actual use by a teacher. \todo{add new requirements list?}
%END

\section*{Results}
The following sections will break down the feedback I have received from participants into the major sections that I wished to evaluate, as set out in section \todo{\S 1.whatever}.
...
\item 12 Runs failed dues to inconsistencies in Indentation\footnote{Python is very strict about indentation rules}
...
\item 3 failed due to python unit testing being done with incorrect arguments\footnote{I am unable to ascertain how this is possible, as all of the unit tests were run in the same manner}
...
\end{itemize}
This would suggest that people are making assumptions about each others code, some correct (such as assuming the other developer would follow the interface), and some incorrect (such as assuming they would structure their code in a certain way). Additionally, it suggests that not all of the participants properly tested their code on the site to make sure their indentation was correct.\todo{reflect that this could be fixed in pipelines - do i do this later?}\par
As a point of interest, during the discussion session, I asked participants how they feel about python's indentation and this revealed that participants in Dubai leaned more towards Tab-based indentation, and in Edinburgh towards spaces. Another issue raised was that in Dubai the primary development OS is Windows (and therefore the CR-LF line endings), versus Linux used in Edinburgh. These consistencies would cause further issues if someone were to try to change the files during testing. To quote of of the participants - ``Python's a mess''. \todo{ref normalising as part of the pipeline}

\subsection{Permission Issues}
Additionally, one respondent raised the issue that they were unable \todo{make sure no repeats - if its in here, dump it from my observations} to see the files of the person they were supposed to be testing. While investigating this I discovered that the issue lay in that the developer had set their solution to be private, which meant that the tester would not be able to see files relating to it. This may be an issue related to usability, which I discuss in section \S \todo{??}.


\subsection{Summary}
This evaluation would suggest that there are many faults in this prototype that would need to be ironed out. Some of these issues would stem from underlying permission management problems (such as people not begin able to see files, which is caused by the website miscommunicating how files should be marked as private or final). There also seems to be some issue with respect to making sure that test case can run successfully - This would need to be fixed as part of the pipeline system  \todo{mention that this wasn't fully implemented before testing, and as part of conducting the signature check on solutions, you could do some sort of check on test cases to make sure they were parseable... perhaps?}.\par
Other than these issues, the behaviour of the site seemed to be consistent with what was expected. There are some places with minor cosmetic issues (such as forms that prompt saying incorrect input before anything has been entered, or mis-aligned html elements), but these did not adversely affect the functionality of the website.\todo{Do i ever want to mention the lack of unit testing? I could mention it and discuss how as this is a prototype the overhead associated with producing tests might be very high. Besides, I think the solution I ended up with is kind of ugly, and re-writing it might allow for some unit -testing kind of situations now I know ore about what the whole thing is supposed to be doing}\par
%end
...
\end{figure}
The Likert scales seem to suggest that most users found the site easy enough to use, and that it responded in a pretty quick manner when interacted with. There did seem to be some issues in the coursework detail page, and its associated file upload page. The feedback view did not appear to have any major issues. Following these questions, respondents were invited to give a more extended description of any issues they had with the site. Some of the responses included:
\begin{itemize}
\item 
\end{itemize}
...
\todo{many people uploadeed multiple times - this might not be clear, may want to offer deletion?}

\subsection{Summary}
\todo{best to do this avfter each EVA-obective, or at the end of the study chapter?}
...
During the discussion session, a question was asked about how the site fared compared to running tests on the command line. The point was made that the website was ``helpful in automating running the tests'', but that it was ``not as quick as bashing something out on the command line''. This would suggest that it may be faster for testers to download file and tinker with them before running their own command line actions.\par

Some comments were made during the discussion session regarding this, however. When discussing the concept of completing programming exercises in pairs vs alone, a Dubai respondent said that Pair programming usually allows for exercises to be complete much faster. Additionally, a student from Edinburgh noted that in Non-anonymous paired exercises, things can be slowed down as you may choose to not give some feedback for fear of hurting feelings. Therefore, a site that offers anonymity should help to provide a safe space for feedback. Also, discussion noted that when you are giving feedback to a person, it is much more direct and that you can share ``insider knowledge'' of shortcuts in programming through discussion and feedback, something that might be more difficult to find by looking for answers online. While these points do not directly relate to the website and peer-testing, I feel they are useful to note as it suggests that the discussion framework offered in website feedback could allow students to share relevant information faster, and more directly, than through simply trying to find it online.\par

Some participants suggested that the ability to receive notifications via email when new feedback was available would be useful, and this would certainly increase the turnaround time for feedback being sent through the website.
...
\todo{Pop the peer-testing and so on discussion related stuff in here, help pad it out a bit - not much else where it can go}\par
...
The participants were asked to rate their opinions on whether or not they agreed with certain website and peer-testing related statements. In figure \todo{which} the results can be seen:\\
\begin{figure}[ht]
...
\end{itemize}
One concern was raised however in that offering such peer-testing might be detrimental to completing some exercises as it opens up a surface for plagiarism. \todo{need to discuss somewhere the advantages and disadvantages of having ``upload'' and ``feedback'' modes}.

\subsection{Anonymity}
The question of anonymty was important in this website, so some survey questions involved checking if the participants were able to guess who they were giving feedback to on the website. Only 1 responded with a maybe, and 1 yes when asked if they knew who wrote the code they were testing. When asked if they knew the location of the person whose code they were testing, 3 said they knew, 1 maybe knew the location. Of course, there was no way to be sure about this \todo{in retrospect, maybe I should have made this an open-answer field and asked them to tell me}. The low number of positive guesses as to who others were suggests that the site does offer anonymity.\par
