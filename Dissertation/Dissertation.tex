...
\section{Risks.......}
\todo{stuff from the thingy  first report}
...
There are many ways that the website could be built. To identify some option \todo{add some narrative...}
\begin{itemize}
\item Languages:
\item PHP
\item Python
...
\item Flask (Python)
\end{itemize}
I decided it would be beneficial to use a language I was familiar with, to allow me to prototype more rapidly, so I decided to choose between some sort of PHP or Python based option, having had previous experience with these. I considered if it would be better to use a framework or start scripting using raw files but decided it would be better to use a framework, as these often have abstractions that can offer standard security measures (for example, user form input validation), as well as other abstractions to help speed up creation of website applications (such as html templating\todo{explain what it is in a footnote?). Given that I would need to restrict the scope of the peer-testing website for this initial prototype I decided, with the suggestion from my supervisor, that it would make sense to use the same language in both the website and the coursework exercises that might be used within the live website, as having the same language features for developing bpth the website and the method for offering testing might be eair to develop. Such a design would ned to be carefully chosen to be modular, as this might increase the temptation to form tightly coupled code. \todo{is this paragraph too long}\par
To this end, I have chosen to develop the application using a python web framework. For initial selection of some frameworks \todo{this may not be in the right place}, I developed...\par
\todo{something something something} so I chose to develop the website using Django. Models? cms? auth management? clean language? unclean language? lightweight? better support? uni compatibility a factor?
\section{Initial Interface Design}
With the focus of this initial prototype being with regards to ensuring the functionality of such a site to be feasible \todo{bad sentence}. barebones ui. 
